...
class Usuari{
	...
	*/
	...
	Crea usuari
	Retorna:
	Una array amb els errors, i les dades introduides per tal de corregir-les
	1: OK
	En cas d'error retorna un array
	ERRORS:
	2 = No és un email
	...
	*/
	public function enviarNouPsw($email) {
		...
		//crear noupassword
			...
			$opt = array(
				'nom' => $nom,
				'link' => "http://".General::base_url."/recover_activate?code=".$codi."&email=".urlencode($email),
				'pass' => $nou_psw,
			);
	...
	*/
	public function activarNouPsw($email,$code) {
		...
		if (mysql_num_rows($sql)<1) 
		{
			...
			mysql_query("UPDATE usuari SET pass = '$new_psw' WHERE id = '$id'") or die(mysql_error());
